...
namespace Naninovel
{
    ...
    public class EditorResourcesEditor
    {
        ...
        /// <param name="pathPrefix">When specified, will add the path before the edited resources names (it won't be visible for the user).</param>
        /// <param name="resourceName">When specified, will draw a single-element editor for resource with the specified name.</param>
        ...
        /// <summary>
        /// Attempts to remove a resources category with the specified ID and all the associated records.
        ...
        private void DrawElement (Rect rect, int index, bool selected, bool focused)
        {
            ...
            if (elementHovered || IsAssetLoadedOrCached(elementGuid) || string.IsNullOrEmpty(elementGuid))
            {
                ...
                if (newObjectValue && (string.IsNullOrEmpty(newNameValue) || !allowRename))
        ...
        {
            if (!allowRename && names.Contains(asset.Asset.name))
            {
                Engine.Warn($"Failed to add '{asset.RelativePath}' asset to '{categoryId}' resources list: asset with the same name is already added.");
                return;
            }
