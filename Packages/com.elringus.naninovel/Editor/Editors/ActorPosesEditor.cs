...
namespace Naninovel
{
    ...
    {
        private static readonly GUIContent nameContent = new("Pose Name",
            "Identifier of the pose. Use it instead of appearance in naninovel script commands to apply the pose.");
        private static readonly GUIContent zeroOpacityTintContent = new("Tint Color",
            "Tint opacity is zero; actor won't be visible. In case you want to hide the actor, set `Visible` to false instead.");
        private static readonly Color warnColor = new(1, 1, .5f);
        private static readonly HashSet<string> overrides = new();
