...
namespace Naninovel
{
    ...
    public class MetadataMapEditor
    {
        ...
        public readonly struct ElementModifiedArgs : IEquatable<ElementModifiedArgs>
        {
            /// <summary>
            ...
            public bool Equals (ElementModifiedArgs other)
            {
            ...
            {
                return obj is ElementModifiedArgs other && Equals(other);
            }

            public override int GetHashCode ()
            {
                return HashCode.Combine(Metadata, (int)ModificationType);
            }
        }
        ...
        private static readonly GUIContent editButtonLabel = new(Engine.LoadInternalResource<Texture2D>("EditMetaIcon"), "Edit actor metadata.");
        ...
        private readonly Dictionary<int, SerializedProperty> idsCache = new();
        private readonly Dictionary<int, SerializedProperty> metasCache = new();
        ...
        /// <summary>
        /// Attempts to start editing metadata mapped to the specified actor ID.
        /// </summary>
        ...
        {
            reorderableList = new(serializedObject, idsProperty, true, true, true, true);
        ...
        private void HandleListElementAdded (ReorderableList list)
        {
            ...
            OnElementModified?.Invoke(new(metaProperty.GetGenericValue<ActorMetadata>(), ElementModificationType.Add));
        }
        ...
        private void HandleListElementRemoved (ReorderableList list)
        {
            ...
            OnElementModified?.Invoke(new(removedElementMetadata, ElementModificationType.Remove));
        }
        ...
        private bool CanAddListElement (ReorderableList list)
        {
            for (int i = 0; i < reorderableList.count; i++)
                if (string.IsNullOrWhiteSpace(GetIdPropertyAt(i).stringValue))
                    return false;
            return true;
        }
        ...
        {
            for (int i = 0; i < reorderableList.count; i++)
                if (GetIdPropertyAt(i).stringValue == key)
                    return false;
            return true;
        }
