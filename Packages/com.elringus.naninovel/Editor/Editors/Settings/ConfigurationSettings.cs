...
namespace Naninovel
{
    ...
    public class ConfigurationSettings : SettingsProvider
    {
        ...
        {
            helpIcon = new(GUIContents.HelpIcon);
            ...
            : base(TypeToSettingsPath(configType), SettingsScope.Project)
        {
            ...
            HelpUri = $"guide/configuration#{ConfigurationType.Name.Replace("Configuration", string.Empty).InsertCamel('-').ToLowerInvariant()}";
        }
        ...
        public override void OnActivate (string searchContext, VisualElement rootElement)
        {
            ...
            SerializedObject = new(Configuration);
        ...
        /// </summary>
        protected virtual Dictionary<string, Action<SerializedProperty>> OverrideConfigurationDrawers () => new();
        ...
        {
            values = UnityEditor.TypeCache.GetTypesDerivedFrom<TImplementation>()
        ...
        private static void SaveConfigurationObject (Configuration configuration)
        {
            ...
            var assetPath = PathUtils.Combine(dirPath, $"{configuration.GetType().Name}.asset");
        ...
        {
            var configTypes = Engine.Types.Configurations.Where(t => t.IsDefined(typeof(Configuration.EditInProjectSettingsAttribute)));
            var editorTypes = UnityEditor.TypeCache.GetTypesDerivedFrom<ConfigurationSettings>();
            ...
            bool IsEditorFor (Type editorType, Type configType)
            {
                ...
                while (type != null)
                {
                    if (type.IsGenericType &&
                        type.GetGenericTypeDefinition() == typeof(ConfigurationSettings<>) &&
                        type.GetGenericArguments()[0] == configType) return true;
                    type = type.BaseType;
                }
                return false;
            }
        ...
        private static SettingsProvider[] CreateProviders ()
        {
            return settingsTypeMap
                .Select(kv => kv.Value == typeof(ConfigurationSettings)
                    ? new ConfigurationSettings(kv.Key)
                    : Activator.CreateInstance(kv.Value) as SettingsProvider).ToArray();
        }
