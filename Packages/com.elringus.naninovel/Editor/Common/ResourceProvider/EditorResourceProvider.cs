...
{
    /// <summary>
    /// Provides resources stored anywhere inside the project's `Assets` folder using specified path to GUID map; works only in the editor.
    /// </summary>
    public class EditorResourceProvider : ResourceProvider
    {
        private readonly Dictionary<string, string> pathToGuidMap = new();

        public void AddResourceGuid (string path, string guid)
        {
            pathToGuidMap[path] = guid;
            LocationsCache.Add(new(path, typeof(UnityEngine.Object)));
        }
