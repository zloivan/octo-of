...
namespace Naninovel
{
    public class ResourcesBuilder
    {
        ...
        public void Build (BuildPlayerOptions options)
        {
            ...
            using var _ = ListPool<Metadata.Resource>.Rent(out var addressableResources);
            AddressableLocator.LocateResources(addressableResources);
            foreach (var resource in addressableResources)
            {
                var assetPath = AssetDatabase.GUIDToAssetPath(resource.AssetId);
                if (assetPath != null && TryGetAssetType(assetPath, out var assetType) &&
                    assetType == typeof(VideoClip) && options.target == BuildTarget.WebGL)
                    ProcessVideoForWebGL($"{resource.Type}/{resource.Path}", assetPath);
            }

            if (config.LabelByScripts)
                addressableBuilder.AssignScriptLabels();
