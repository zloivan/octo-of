...
namespace Naninovel
{
    public class ConsoleGUI : MonoBehaviour
    {
        ...
        /// </summary>
        public static Color BackgroundColor { get; set; } = new(0, 0, 0, .65f);
        /// <summary>
        ...
        /// <summary>
        /// Whether to ignore typed character.
        /// </summary>
        public static Predicate<char> IgnoreCharacter { get; set; } = c => c == '`';
        /// <summary>
        /// Invoked when console input is shown.
        /// </summary>
        public static Action OnShow { get; set; }
        /// <summary>
        /// Invoked when console input is hidden.
        /// </summary>
        public static Action OnHide { get; set; }
        /// <summary>
        /// Whether the console is currently shown.
        /// </summary>
        public static bool Visible => guiProxy && guiProxy.enabled;
        ...
        private static readonly List<string> inputBuffer = new();
        ...
        public static void Initialize (Dictionary<string, MethodInfo> commands = null)
        {
            ...
            Scale = ScaleByResolution ? new(Mathf.Max(Screen.width / 1920f, 1), Mathf.Max(Screen.height / 1080f, 1)) : Vector2.one;
            Size = new(Screen.width - 125 * Scale.x, 25 * Scale.y);
            consoleRect = new(0, 0, Size.x, Size.y);
            execButtonRect = new(Size.x, 0, 75 * Scale.x, Size.y);
            closeButtonRect = new(Screen.width - 050 * Scale.x, 0, 50 * Scale.x, Size.y);
            Style = new() {
                richText = true,
                normal = new() { background = Texture2D.whiteTexture, textColor = Color.white },
        ...
        private void Update ()
        {
            if (ToggleKey == KeyCode.None || !Application.isPlaying) return;
        ...
        {
            if (Event.current.isKey && IgnoreCharacter(Event.current.character))
                Event.current.Use();
