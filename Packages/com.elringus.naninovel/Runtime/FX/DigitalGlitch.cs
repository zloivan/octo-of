...
namespace Naninovel.FX
{
    public class DigitalGlitch : MonoBehaviour, Spawn.IParameterized, Spawn.IAwaitable
    {
        ...
        protected ISpawnManager SpawnManager => Engine.GetServiceOrErr<ISpawnManager>();
        ...
        private class CameraComponent : MonoBehaviour
        {
            ...
            public Color TintColor = new(.2f, .2f, 0f, 0f);
            ...
            private Material Material
            {
                get
                {
                    if (material is null)
                    {
                        ...
                        material.SetTextureScale(glitchTex, new(Screen.width / (float)GlitchTexture.width, Screen.height / (float)GlitchTexture.height));
            ...
            private void OnRenderImage (RenderTexture source, RenderTexture destination)
            {
                ...
                glitchUp += Engine.Time.DeltaTime * Intensity;
                glitchDown += Engine.Time.DeltaTime * Intensity;
                flicker += Engine.Time.DeltaTime * Intensity;

                if (flicker > flickerTime)
                {
                    Material.SetFloat(filterRadius, Random.Range(-3f, 3f) * Intensity * ShiftAmount);
                    Material.SetTextureOffset(glitchTex, new(Random.Range(-3f, 3f), Random.Range(-3f, 3f)));
                    flicker = 0;
                    flickerTime = RandomGlitchFrequency ? Random.value : 1f - GlitchFrequency;
                }
