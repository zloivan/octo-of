...
namespace Naninovel
{
    ...
    public class TransientSaveSlotManager<TData> : TransientSaveSlotManager, ISaveSlotManager<TData> where TData : new()
    {
        ...
        protected virtual Dictionary<string, TData> SlotToData { get; } = new();

        public virtual void SaveBlocking (string slotId, TData data)
        {
        ...
        public virtual TData LoadBlocking (string slotId)
        {
        ...
        public virtual TData LoadOrDefaultBlocking (string slotId)
        {
            if (!SaveSlotExists(slotId)) Save(slotId, DefaultFactory());
            return LoadBlocking(slotId);
        }

        public virtual UniTask Save (string slotId, TData data)
        {
            SaveBlocking(slotId, data);
            return UniTask.CompletedTask;
        }

        public virtual UniTask<TData> Load (string slotId) => UniTask.FromResult(LoadBlocking(slotId));
        public virtual UniTask<TData> LoadOrDefault (string slotId) => UniTask.FromResult(LoadOrDefaultBlocking(slotId));
        ...
        public override void RenameSaveSlot (string sourceSlotId, string destSlotId)
        {
            ...
            Save(destSlotId, LoadBlocking(sourceSlotId));
