...
namespace Naninovel
{
    ...
    public class PlayerPrefsSaveSlotManager<TData> : PlayerPrefsSaveSlotManager, ISaveSlotManager<TData> where TData : class, new()
    {
        ...
        public virtual async UniTask Save (string slotId, TData data)
        {
        ...
        public virtual void SaveBlocking (string slotId, TData data)
        {
            saveInProgress = true;
            InvokeOnBeforeSave(slotId);
            SerializeDataBlocking(slotId, data);
            InvokeOnSaved(slotId);
            saveInProgress = false;
        }

        public virtual async UniTask<TData> Load (string slotId)
        {
        ...
        public virtual async UniTask<TData> LoadOrDefault (string slotId)
        {
            if (!SaveSlotExists(slotId))
        ...
        protected virtual async UniTask SerializeData (string slotId, TData data)
        {
        ...
        protected virtual void SerializeDataBlocking (string slotId, TData data)
        {
            var jsonData = JsonUtility.ToJson(data, PrettifyJson);
            var slotKey = SlotIdToKey(slotId);
        ...
        protected virtual async UniTask<TData> DeserializeData (string slotId)
        {
            var slotKey = SlotIdToKey(slotId);
