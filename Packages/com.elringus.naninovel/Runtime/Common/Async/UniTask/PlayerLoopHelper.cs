...
namespace Naninovel.Async
{
    ...
    {
        private static readonly ContinuationQueue ThrowMarkerContinuationQueue = new(PlayerLoopTiming.Initialization);
        private static readonly PlayerLoopRunner ThrowMarkerPlayerLoopRunner = new(PlayerLoopTiming.Initialization);
            ...
            int index, bool injectOnFirst, Type loopRunnerYieldType, Type loopRunnerType, PlayerLoopTiming playerLoopTiming)
        {
            ...
            if ((injectTimings & targetTimings) == targetTimings)
            {
                copyList[i].subSystemList = InsertRunner(copyList[i], injectOnFirst,
                    loopRunnerYieldType, yielders[index] = new(playerLoopTiming),
                    loopRunnerType, runners[index] = new(playerLoopTiming));
            }
            else
            {
