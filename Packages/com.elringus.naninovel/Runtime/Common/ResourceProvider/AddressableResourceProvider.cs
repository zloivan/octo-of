...
namespace Naninovel
{
    public class AddressableResourceProvider : ResourceProvider
    {
        ...
        {
            locations ??= await LoadAllLocations();
            return await base.LoadResource<T>(path);
        }

        public override async UniTask<IReadOnlyCollection<string>> LocateResources<T> (string path)
        {
            locations ??= await LoadAllLocations();
            return await base.LocateResources<T>(path);
        }

        public override async UniTask<IReadOnlyCollection<Folder>> LocateFolders (string path)
        {
            locations ??= await LoadAllLocations();
            return await base.LocateFolders(path);
        }
        ...
        protected virtual void CacheLocations (IEnumerable<IResourceLocation> locations)
        {
            foreach (var location in locations)
            {
                ...
                LocationsCache.Add(new(path, location.ResourceType));
            }
        }
    }
}
