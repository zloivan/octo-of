...
namespace Naninovel
{
    ...
    public class ProjectResourceProvider : ResourceProvider
    {
        public class TypeRedirector
        {
            ...
            public virtual async UniTask<TSource> ToSourceAsync<TSource> (object obj, string path)
            {
                return (TSource)await RedirectToSourceConverter.Convert(obj, path);
            }

            public virtual TSource ToSource<TSource> (object obj, string path)
            {
                return (TSource)RedirectToSourceConverter.ConvertBlocking(obj, path);
            }
        }
        ...
        public ProjectResourceProvider (string rootPath = null)
        {
            ...
            foreach (var kv in projectResources)
                LocationsCache.Add(new(kv.Key, kv.Value));
