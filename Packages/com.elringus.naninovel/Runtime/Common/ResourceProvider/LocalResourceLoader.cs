        ...
        where TResource : UnityEngine.Object
    {
        ...
        public override async UniTask Run ()
        {
            ...
            foreach (var representation in converter.Representations)
            {
                ...
                rawData = await IOUtils.ReadFile(fullPath);
                break;
            }
            ...
            var obj = await selectedConverter.Convert(rawData, Path);
            var result = new Resource<TResource>(Path, obj);

            SetResult(result);

            logAction?.Invoke($"Resource '{Path}' loaded {StringUtils.FormatFileSize(rawData.Length)} over {Engine.Time.UnscaledTime - startTime:0.###} seconds.");
        }
    }
}
