...
namespace Naninovel
{
        ...
        where TSlot : ScriptableGridSlot
    {
        ...
        public virtual async UniTask Initialize (int itemsCount)
        {
            ItemsCount = itemsCount;
            Slots = await CreateSlots();
            FocusOnNavigation = Slots[^1].gameObject;
            SelectPage(1);
            if (PaginationPanel)
        ...
        private async UniTask<TSlot[]> CreateSlots ()
        {
            using var _ = ListPool<UniTask>.Rent(out var tasks);
            var slots = new TSlot[ItemsPerPage];
            for (int i = 0; i < ItemsPerPage; i++)
                tasks.Add(CreateSlot(slots, i));
            await UniTask.WhenAll(tasks);
            // Parent after the async instantiation to preserve slot order inside the container.
            for (var i = 0; i < slots.Length; i++)
                slots[i].transform.SetParent(transform, false);
            return slots;
        }

        private async UniTask CreateSlot (TSlot[] slots, int idx)
        {
            InitializeSlot(slots[idx] = await Engine.Instantiate<TSlot>(SlotPrototype));
        }
    }
}
