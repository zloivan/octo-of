...
namespace Naninovel
{
    public static class MathUtils
    {
        ...
        {
            return new(ClampAngle(vector3.x), ClampAngle(vector3.y), ClampAngle(vector3.z));
        }
        ...
        {
            return new(rect.position * scale, rect.size * scale);
        }

        public static Rect Scale (this Rect rect, Vector2 scale)
        {
            return new(new(rect.position.x * scale.x, rect.position.y * scale.y),
                new(rect.size.x * scale.x, rect.size.y * scale.y));
        }
        ...
        {
            return new(rect.position - Vector2.one * cropAmount, rect.size + Vector2.one * cropAmount * 2f);
        }
