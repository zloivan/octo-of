...
namespace Naninovel
{
    public static class TransformUtils
    {
        ...
        {
            if (local) trs.localPosition = new(x, trs.localPosition.y, trs.localPosition.z);
            else trs.position = new(x, trs.position.y, trs.position.z);
        }

        public static void SetPosY (this Transform trs, float y, bool local = false)
        {
            if (local) trs.localPosition = new(trs.localPosition.x, y, trs.localPosition.z);
            else trs.position = new(trs.position.x, y, trs.position.z);
        }

        public static void SetPosZ (this Transform trs, float z, bool local = false)
        {
            if (local) trs.localPosition = new(trs.localPosition.x, trs.localPosition.y, z);
            else trs.position = new(trs.position.x, trs.position.y, z);
        }
