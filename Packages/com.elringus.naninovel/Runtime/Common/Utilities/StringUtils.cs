...
namespace Naninovel
{
    ...
    public static class StringUtils
    {
        ...
        /// <summary>
        /// Checks whether specified string contains any line break characters (platform-agnostic).
        /// </summary>
        ...
        /// <summary>
        /// Performs <see cref="string.StartsWith(string)"/> and <see cref="string.EndsWith(string)"/> with the specified match.
        /// </summary>
        ...
        /// <summary>
        /// Attempts to extract a subset of the specified <paramref name="source"/> string, starting at
        ...
        /// <summary>
        /// Modifies the string by inserting specified char (space by default) based on camel case scheme; eg, `SomeFancyName` becomes `Some Fancy Name`.
        /// </summary>
        ...
        public static async UniTask<byte[]> ZipStringAsync (string content)
        {
            ...
            using var output = new MemoryStream();
            await using (var gzip = new DeflateStream(output, CompressionMode.Compress))
            {
        ...
        public static async UniTask<string> UnzipStringAsync (byte[] content)
        {
            ...
            using var inputStream = new MemoryStream(content);
            await using var gzip = new DeflateStream(inputStream, CompressionMode.Decompress);
            using var reader = new StreamReader(gzip, Encoding.UTF8);
