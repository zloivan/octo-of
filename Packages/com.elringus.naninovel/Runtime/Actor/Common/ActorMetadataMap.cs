...
namespace Naninovel
{
        ...
        where TMeta : ActorMetadata
    {
        ...
        [SerializeField] private string[] ids = Array.Empty<string>();
        [SerializeField] private TMeta[] metas = Array.Empty<TMeta>();
        ...
        public override List<string> GetAllIds () => new(ids);

        public List<TMeta> GetAllMetas () => new(metas);
        ...
        {
            if (!ContainsId(sourceId)) throw new Error($"Failed to move `{sourceId}` actor record: actor with ID not found.");
            if (ContainsId(destinationId)) throw new Error($"Failed to move `{sourceId}` actor record to `{destinationId}`: another actor with same ID exists.");
